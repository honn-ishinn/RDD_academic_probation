...
&nbsp; &nbsp; As the GPA is an continuous variable from 0.0 to 4.0, I would use the linear regression to estimate the probation impact on student annual GPA in the second academic year. 
...
&nbsp; &nbsp; Although the original data obtained from Lindo's paper contains detailed student information to carry on this study, I find the data dirty through following examples:

* The original data actually records student graduation information by Year 4, Year 5 and Year 6. However, there are enormous missing values on these graduation related data (14345 NAs on Year 4, 19781 NAs on Year 5, 24605 NAs on Year 6 out of 44362 observations). Besides, even some students are recorded to graduate by Year 4 but not recorded by Year 5 or Year 6. So I refrained to explore the probation impact on graduation by Year 5 and Year 6 because of such data inconsistency, even though Lindo did relevant analysis in their original paper.
...
&nbsp; &nbsp; Above examples demonstrate a relatively poor quality of data for analysis. And these dirty data may also affect accuracy of study results.

## Comment and Future Works

&nbsp; &nbsp; As a replication study, this paper does not use the exact same model as in the original paper. The original paper subsets the data into different subgroups by student gender, highschool performance and first language and uses RDD to examine the probation impact on each subgroup, while this paper includes all of those student characteristics into one model and uses RDD to examine the probation impact as a whole. Even though the analysis approach is slightly different, I believe the model used in this study is reasonable and could draw appropriate findings.

&nbsp; &nbsp; Surely, there must be other characteristics affecting student responses towards academic probation as well (e.g., family background, income, personal trait, etc.).

Weaknesses and next steps [@cite_success]should also be included.

The university should not rely on the academic probation policy itself to hope for improvement of student academic performance but rather introducing some study programs to help improving the student 
